...
- (void)removeHeaderAtIndex:(NSInteger)index
{
    ...
    [selectionIndexes enumerateIndexesUsingBlock:^(NSUInteger idx, BOOL *stop)
     {
        NSLog(@"idx before:%lu",(unsigned long)idx);
        if (idx == index) return;
        if (idx > index) idx--;
...
- (void)setSelectionIndexes:(NSIndexSet *)aSelectionIndexes
{
    ...
    if ([delegate respondsToSelector:@selector(accordion:previosSelection:)]) {
        
        [delegate accordion:self previousSelection:self.selectionIndexes];
    }
...
-(void)replaceHeader:(id)aHeader withView:(id)aView withTag:(int)subViewTag
{
    
    if ([aHeader respondsToSelector:@selector(addTarget:action:forControlEvents:)])
     {
     [aHeader setTag:subViewTag];
     [aHeader addTarget:self action:@selector(nextButtonClicked:) forControlEvents:UIControlEventTouchUpInside];
     }
...
- (void)nextButtonClicked:(id)sender
{
    ...
    [self setSelectionIndexes:[NSIndexSet indexSetWithIndex:[sender tag]]];
